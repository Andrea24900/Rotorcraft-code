...
def build_mass_matrix(
    ...
    Returns:
        Mass matrix M (a function handle in Omega,t)
    ...
    def mass_matrix_rot(time: float, rotor_omega:float) -> np.ndarray:
        ...
        mass_matrix_rot = np.block([[upper_left,upper_right],[lower_left,lower_right]])

        return mass_matrix_rot

    def mass_matrix(problem,time,rotor_omega):

        # rotation of the matrix inthe proper frame
        mbc_matrix = build_mbc_matrix(problem, time, rotor_omega)
        mass_matrix = np.transpose(mbc_matrix)@mass_matrix_rot@mbc_matrix
...
def build_damping_matrix(
    ...
    def damping_matrix_rot(time:float ,rotor_omega:float):
        ...
        upper_left = np.zeros(num_blades)
        # Lower right
        lower_right = np.array([
            [hub_damping_x,0],
            [0,hub_damping_y]
        ...
        lower_left = blade_static_moment*np.vstack([upper_vec,lower_vec])
        #generation of mass matrix (partially in rotating frame)
        damping_matrix_rot = np.block([[upper_left,upper_right],[lower_left,lower_right]])
        
        # 2) C with the addition of the dampers
        damping_matrix_addition = np.zeros((num_blades,num_blades))
        if problem.damper_connection == "H2B":
            diagonal = damping_activation_H2B(problem)
            damping_matrix_addition[0:num_blades-1,0:num_blades-1] = np.diag(diagonal)
        #elif problem.damper_connection == "B2B":
            # B2B damper connection logic to be implemented
            
        # summation of matrices
        damping_matrix_rot = damping_matrix_rot(time,rotor_omega)+damping_matrix_addition
        # 3) transformation through MBC
        return damping_matrix_rot
    ...
    def damping_matrix_C(time,rotor_omega):
        ...
        damping_matrix_C = np.transpose(mbc_matrix)@damping_matrix_rot@mbc_matrix + 2*np.transpose(mbc_matrix)@mass_matrix_rot@mbc_matrix_derivative
        return damping_matrix_C
    ...
    def stiffness_matrix_K(time:float ,rotor_omega:float):
        ...
        for blade_index in range(1,num_blades+1):
            ...
            upper_vec[blade_index-1]=2*rotor_omega*blade_static_moment*np.cos(psi(rotor_omega,time))
            lower_vec[blade_index-1]=2*rotor_omega*blade_static_moment*np.sin(psi(rotor_omega,time))
        lower_left = -rotor_omega**2*blade_static_moment*np.vstack([upper_vec,lower_vec])
        #generation of mass matrix (partially in rotating frame)
        stiffness_matrix_rot = np.block([[upper_left,upper_right],[lower_left,lower_right]])
            
        # 2) K with the addition of the dampers
        stiffness_matrix_addition = np.zeros((num_blades,num_blades))
        if problem.damper_connection == "H2B":
            diagonal = nominal_stiffness*np.ones(num_blades)
            stiffness_matrix_addition[0:num_blades-1,0:num_blades-1] = np.diag(diagonal)
        #elif problem.damper_connection == "B2B":
                # B2B damper connection logic to be implemented
                
            # summation of matrices
        stiffness_matrix_rot = stiffness_matrix_rot(time,rotor_omega)+stiffness_matrix_addition
            # 3) transformation through MBC
            # build the MBC matrices
        mbc_matrix = build_mbc_matrix(problem,time,rotor_omega)
        mbc_matrix_derivative = build_mbc_matrix_derivative(problem,time,rotor_omega)
        mbc_matrix_derivative_second = build_mbc_matrix_derivative_second(problem,time,rotor_omega)
            # complete the tranformation
        stiffness_matrix_K = np.transpose(mbc_matrix)@stiffness_matrix_rot@mbc_matrix+ np.transpose(mbc_matrix)@damping_matrix_rot@mbc_matrix_derivative + np.transpose(mbc_matrix)@mass_matrix_rot@mbc_matrix_derivative_second
        return stiffness_matrix_K
